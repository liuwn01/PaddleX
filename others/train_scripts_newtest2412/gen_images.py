...
def calculate_image_size(generated_str, start_x, start_y, spacing, font_size):
    ...
    for char in generated_str:
        ...
        mapping_font = FONT_MAPPING.get(char, config.Default_Font)
...
def gen_images_vertical(task, optIndex):
    ...
    real_y = start_y
    for char in generated_str:
        target_font = FONT_MAPPING.get(char, config.Default_Font)
        real_char = IMAGE_CHARS_MAPPING.get(char, {"word": char})["word"]
