...
with open(f"{root_folder}/result_paddleocr_{TINDEX}_sorted.csv", 'w', encoding='utf-8') as w, open(config.Rollback_JSON, 'r', encoding='utf-8') as r:
    ...
    for c in list(set(char_list)):
        real_char = EXCEPT_CHARS_RALLBACK.get(c, {"word": c})["word"]
        target_list.append(real_char)
