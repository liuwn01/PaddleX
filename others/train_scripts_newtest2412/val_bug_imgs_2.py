...
def parse_ocr_result(ocrresult, source_image_path):
    ...
    for d in merged_data:
        ...
        text_x = start_x = points_tuple[0][0]
        text_y = start_y = points_tuple[0][1]
        for index, c in enumerate(list(d["rec_text"])):
            target_font = ImageFont.truetype(FONT_MAPPING.get(c, "arial.ttf"), font_size)
            rollback_c = IMAGE_CHARS_MAPPING.get(c, {"word": c})["word"]
            bbox = draw.textbbox((text_x, text_y), rollback_c, font=target_font)
            draw.text((text_x, text_y), rollback_c, fill="black", font=target_font, spacing=0)
            text_x = text_x + (bbox[2] - bbox[0])
            text_y = start_y
...
pipeline = create_pipeline(pipeline=config.Pipeline_OCR_yaml)
        ...
        if (f_name.endswith(".jpg") or f_name.endswith(".png")) and (not 'parsed' in f_name):
            ...
            for res in output:
                ...
                for c in list(predict_text):
                    target_list.append(IMAGE_CHARS_MAPPING.get(c, {"word": c})["word"])
